...
// API endpoint to get the redirect number
app.get("/api/redirect-number", async (req, res) => {
  try {
    const redirect = await prisma.redirect.findFirst();
    if (redirect && redirect.number) {
      res.json({ number: redirect.number });
    } else {
      res.json({ number: "" }); // Return empty string if no number is set
    }
  } catch (error) {
    console.error("Error fetching redirect number:", error);
    res.status(500).json({ error: "Failed to fetch redirect number" });
  }
});

// API endpoint to set/update the redirect number
app.post("/api/redirect-number", async (req, res) => {
  try {
    ...
    let updatedRedirect;
    if (existingRedirect) {
      updatedRedirect = await prisma.redirect.update({
        where: { id: existingRedirect.id },
        data: { number: number },
      });
    } else {
      updatedRedirect = await prisma.redirect.create({
        data: { number: number },
      });
    }
    res.json(updatedRedirect);
  } catch (error) {
    console.error("Error updating redirect number:", error);
    res.status(500).json({ error: "Failed to update redirect number" });
  }
});
...
app.post("/inbound", async (req, res) => {
  try {
    ...
    if (redirectRecord && redirectRecord.number) {
      ...
      if (process.env.FORWARD_TO_NUMBER && !redirectRecord) {
        try {
          await prisma.redirect.create({
            data: { number: process.env.FORWARD_TO_NUMBER },
          });
          console.log("Created default redirect record from ENV variable.");
        } catch (dbError) {
          console.error("Could not create default redirect record:", dbError);
        }
      }
    }
  ...
  try {
    ...
    if (isClientCall) {
      ...
      if (redirectRecord && redirectRecord.number) {
        forwardToNumber = redirectRecord.number;
        console.log(
          "Found redirect number in DB for root path:",
          forwardToNumber
        );
      } else {
        forwardToNumber = process.env.FORWARD_TO_NUMBER; // Fallback to ENV
        console.log(
          "Using ENV redirect number for root path (or undefined if not set):",
          forwardToNumber
        );
        // Optionally, create a default redirect record if none exists and env var is used
        if (process.env.FORWARD_TO_NUMBER && !redirectRecord) {
          try {
            ...
            console.log(
              "Created default redirect record from ENV variable for root path."
            );
          } catch (dbError) {
            console.error(
              "Could not create default redirect record for root path:",
              dbError
            );
          }
        }
      ...
      if (forwardToNumber) {
        ...
        console.log("No forward number available. Hanging up.");
        twiml.say(
...
app.get("/test-transcribe", async (req, res) => {
  try {
    ...
    if (!mp3File) {
      return res
        .status(404)
        .json({ error: "No MP3 files found in recordings directory" });
    }
    ...
    const formattedCalls = calls.map((call) => {
      ...
      const formatDate = (date) => {
        ...
        const istDate = new Date(
          callDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
        );
...
app.get("/api/phone-numbers/:id/calls", async (req, res) => {
  try {
    ...
    const numberDetails = {
      ...
      lastCalled: phoneNumber.lastCalled
        ? new Date(
            phoneNumber.lastCalled.toLocaleString("en-US", {
              timeZone: "Asia/Kolkata",
            })
    ...
    const callRecordings = phoneNumber.calls.map((call) => {
      const startTime = new Date(
        call.startTime.toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
      );
      ...
      const questions =
        call.recording?.transcription?.answers.map((a) => ({
          id: a.question.id,
...
app.get("/api/recordings/:recordingId", async (req, res) => {
  try {
    ...
    res.setHeader(
      ...
      "https://call-managment.vercel.app"
    );
    ...
    if (range) {
      ...
      res.setHeader(
        "Content-Range",
        `bytes ${start}-${end}/${response.ContentLength}`
      );
...
app.post("/process-transcription", async (req, res) => {
  try {
    ...
    console.log(
      `Found transcription with text length: ${transcription.text.length}`
    );
    console.log(`Existing questions: ${transcription.questions.length}`);

    // Start analysis in the background
    analyzeTranscription(transcriptionId, transcription.text).catch((error) => {
      console.error("Background analysis failed:", error);
    });
